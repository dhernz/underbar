...
_.each = function(collection, iterator) {
  ...
  _.reject = function(collection, test) {
    ...
    var result = [];
    var arrPassed = _.filter(collection,test);
    _.each(collection,function(value){
        if(_.indexOf(arrPassed,value) == -1 ){
          result.push(value);
        }
    })
    return result;
  ...
  _.reduce = function(arr, callback, initial) {
    ...
    _.each(arr,function(elem,idx,val){
        accumulator = callback( accumulator, elem, idx,arr);
    });
    return accumulator;
